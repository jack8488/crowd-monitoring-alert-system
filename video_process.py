...
def _record_abnormal_activity(abnormal_writer, frame_count, abnormal_individuals, human_count):
    # Write abnormal activity data: frame number, number of abnormal individuals, total individuals
    data = [frame_count, len(abnormal_individuals), human_count]
    abnormal_writer.writerow(data)

def _save_abnormal_screenshot(frame, frame_count, screenshot_count, last_screenshot_frame):
    # Use the specified assets directory
    screenshot_dir = r"C:\Users\mrroh\Desktop\ai edit\assets"
    os.makedirs(screenshot_dir, exist_ok=True)
    # Minimum frame gap between screenshots (e.g., 1.27 seconds at 30 FPS ≈ 38 frames)
    MIN_FRAME_GAP = int(38)  # Adjust based on your video FPS
    if screenshot_count < 11 and (frame_count - last_screenshot_frame >= MIN_FRAME_GAP):
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{screenshot_dir}/assets_abnormal_{timestamp}_frame_{frame_count}.jpg"
        cv2.imwrite(filename, frame)
        print(f"Saved screenshot to {filename}")
        return screenshot_count + 1, frame_count
    return screenshot_count, last_screenshot_frame
...
def video_process(cap, frame_size, net, ln, encoder, tracker, movement_data_writer, crowd_data_writer):
    ...
    last_screenshot_frame = -1  # Track the last frame a screenshot was taken

    RE = False
    ABNORMAL = False

    # Open abnormal activity file for writing
    if DATA_RECORD and ABNORMAL_CHECK:
        os.makedirs(os.path.dirname(ABNORMAL_ACTIVITY_FILE), exist_ok=True)
        abnormal_writer = csv.writer(open(ABNORMAL_ACTIVITY_FILE, 'a', newline=''))
        # Write header if file is new
        if os.path.getsize(ABNORMAL_ACTIVITY_FILE) == 0:
            abnormal_writer.writerow(["Frame", "Abnormal_Count", "Total_Count"])
    ...
    while True:
        ...
        if SHOW_PROCESSING_OUTPUT or SHOW_DETECT or SD_CHECK or RE_CHECK or ABNORMAL_CHECK:
            ...
            if len(humans_detected) > ABNORMAL_MIN_PEOPLE:
                if len(abnormal_individual) / len(humans_detected) > ABNORMAL_THRESH:
                    ABNORMAL = True
                    if DATA_RECORD and ABNORMAL_CHECK:
                        _record_abnormal_activity(abnormal_writer, frame_count, abnormal_individual, len(humans_detected))
        ...
        # Place abnormal activity warning and take screenshot
        if ABNORMAL_CHECK:
            if ABNORMAL:
                # Warning stays on screen for 10 frames
                ab_warning_timeout = 10
                # Draw blue boxes over the abnormally behaved detection if abnormal activity detected
            ...
            if ab_warning_timeout > 0:
                if display_frame_count % 3 != 0:
                    ...
                    # Take screenshot only when alert is shown and limit to 11
                    screenshot_count, last_screenshot_frame = _save_abnormal_screenshot(frame, frame_count, screenshot_count, last_screenshot_frame)
